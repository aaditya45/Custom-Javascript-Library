...
function Modal(cref) {
  ...
  for (
    contentIndex = 0;
    contentIndex < contentParentReference.children.length;
    contentIndex++
  ) {
  ...
  modalMaskDivision.classList.add("custom_modalMask");
  var modalDivision = document.createElement("div");
  modalDivision.style.display = "none";
  modalDivision.classList.add("custom_modal");
  ...
  if (contentReference.hasAttribute("size")) {
    ...
    if (splitPoint == -1)
      throw "In case of modal size should be specified as widthXheight";
    if (splitPoint == 0 || splitPoint == sz.length - 1)
      throw "In case of modal size should be specified as widthXheight";
  ...
  contentDivision.style.height =
    modalDivision.style.height.substring(
      0,
      modalDivision.style.height.length - 2
    ) -
    130 +
    "px";
  ...
  if (contentReference.hasAttribute("closeButton")) {
    ...
    if (cb.toUpperCase() == "TRUE") {
      ...
      closeButtonSpan.classList.add("custom_closeButton");
...
$$$.accordianHeadingClicked = function (accordianIndex, panelIndex) {
  if ($$$.model.accordians[accordianIndex].expandedIndex != -1)
    $$$.model.accordians[accordianIndex].panels[
      $$$.model.accordians[accordianIndex].expandedIndex
    ].style.display = "none";
  $$$.model.accordians[accordianIndex].panels[panelIndex + 1].style.display =
    $$$.model.accordians[accordianIndex].panels[panelIndex + 1].oldDisplay;
...
$$$.toAccordian = function (accord) {
  ...
  if (panels.length % 2 != 0)
    throw "Heading and division malformed to create accordian";
...
$$$.onDocumentLoaded = function (func) {
  if (typeof func != "function")
    throw (
      "Excepted function, found" + typeof func + " is call to onDocumentLoaded"
    );
...
function TMJRockElement(element) {
  ...
  this.fillComboBox = function (jsonObject) {
    ...
    if (!(dataSource instanceof Array))
      throw "Value of dataSource property should be of collection type";
    if (dataSource.length == 0)
      throw "Length of collection against dataSource property can not be 0";

    let text = jsonObject["text"];
    if (!text) throw "text property should exist in call to ajax";
    if (typeof text != "string")
      throw "text property should be string of type in call to ajax";
    if (!dataSource[0][text])
      throw "value of text property should exist as a property in elements of collection against dataSource property";

    let value = jsonObject["value"];
    if (!value) throw "value propety should exist in call to ajax";
    if (typeof value != "string")
      throw "value property should be of string type in call to ajax";
    if (!dataSource[0][value])
      throw "value of value property should exist as a property in elements of collection against dataSource property";
    ...
    if (firstOption) {
      ...
      if (!firstOptionText)
        throw "text property should exist in json against firstOption property in call to ajax";
      if (typeof firstOptionText != "string")
        throw "text property in json object against firstOption property should be of string type in call to ajax";

      let firstOptionValue = firstOption["value"];
      if (!firstOptionValue)
        throw "Value property should exist in json against firstOption property in call to ajax";
      if (typeof firstOptionValue != "string")
        throw "Value property in json object against firstOption property should be of string type in call to ajax";
...
$$$.ajax = function (jsonObject) {
  ...
  if (jsonObject["methodType"]) {
    ...
    if (["GET", "POST"].includes(methodType) == false)
      throw "methodType should be of GET/POST in call to ajax";
  }
      ...
      isValid: function (dict) {
        ...
        for (var i = 0; i < keys.length; i++) {
          ...
          if (formElement.type == "text") {
            ...
            if (
              dictElementKeys.includes("required") &&
              dictElement["required"] &&
              formElement.value.trim().length == 0
            ) {
              document.getElementById(dictElement["error-pane"]).innerHTML =
                dictElement.error["required"];
              valid = false;
              if (firstInvalidInput == null) firstInvalidInput = formElement;
            }
            if (
              dictElementKeys.includes("max-length") &&
              formElement.value.trim().length > dictElement["max-length"]
            ) {
              document.getElementById(dictElement["error-pane"]).innerHTML =
                dictElement.error["max-length"];
              valid = false;
              if (firstInvalidInput == null) firstInvalidInput = formElement;
            }
          }
          ...
          if (formElement.type == "select-one") {
            document.getElementById(dictElement["error-pane"]).innerHTML = "";
            if (
              dictElementKeys.includes("invalid") &&
              formElement.value == dictElement["invalid"]
            ) {
              //alert(dictElement["error-pane"]);
              //alert(dictElement.error["required"]);
              document.getElementById(dictElement["error-pane"]).innerHTML =
                dictElement.error["invalid"];
              valid = false;
              if (firstInvalidInput == null) firstInvalidInput = formElement;
            }
          }
          ...
          if (formElement.type == "checkbox") {
            if (
              dictElementKeys.includes("required-state") &&
              dictElement["required-state"] != formElement.checked &&
              dictElement["display-alert"]
            ) {
